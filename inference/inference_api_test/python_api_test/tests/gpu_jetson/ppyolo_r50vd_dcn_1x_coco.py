...
if __name__ == "__main__":
    ...
    try:
        ...
        os.system('echo "GPU ppyolo_r50vd_dcn_1x_coco inference error" >> log.txt')
    else:
        max_diff = np.max(np.abs(data - ir_optim_data))
        if max_diff < 1e-6:
            print('GPU ppyolo_r50vd_dcn_1x_coco ok')
        else:
            print('GPU ppyolo_r50vd_dcn_1x_coco failed, diff is ', max_diff)
            os.system('echo "GPU ppyolo_r50vd_dcn_1x_coco failed,diff is "' + str(max_diff) + '>> log.txt')
