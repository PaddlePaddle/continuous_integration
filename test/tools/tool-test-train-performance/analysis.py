...
def parse_args():
    ...
    parser.add_argument(
        '--FrameName', type=str, default="paddlepaddle", help='{paddlepaddle}')
        ...
        with open(self.filename, "r") as f_object:
            ...
            for line in lines:
                ...
                try:
                    ...
                    if args.position:  # 明确切分后result的位置
...
if __name__ == "__main__":
    ...
    run_info["FrameName"] = args.FrameName

    # 计算指标
    try:
        if args.index == 1:  # 计算速度指标
        ...
        traceback.print_exc()
    print("ips: {} images/s".format(json.dumps(run_info["FINAL_RESULT"]))
