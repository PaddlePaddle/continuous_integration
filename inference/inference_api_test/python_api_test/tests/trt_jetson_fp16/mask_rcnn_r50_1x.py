...
def parse_args():
    ...
    parser.add_argument("--model_file",default='../../Data/python-model-infer/Detection/mask_rcnn_r50_1x/model/__model__' ,type=str, help="model filename")
    parser.add_argument("--params_file", default='../../Data/python-model-infer/Detection/mask_rcnn_r50_1x/model/__params__',type=str, help="parameter filename")
...
if __name__ == "__main__":
    try:
        ...
        os.system('echo "TensorRT FP16 mask_rcnn_r50_1x inference error" >> log.txt')
    else:
        max_diff = np.max(np.abs(data - ir_optim_data))
        if max_diff < 1e-6:
            print('TensorRT FP16 mask_rcnn_r50_1x ok')
        else:
            print('TensorRT FP16 mask_rcnn_r50_1x failed, diff is ', max_diff)
            os.system('echo "TensorRT FP16 mask_rcnn_r50_1x failed,diff is "' + str(max_diff) + '>> log.txt')
