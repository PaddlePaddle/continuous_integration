...
def test_array_length():
    ...
    with fluid.unique_name.guard():
        with fluid.program_guard(train_program, startup_program):
            ...
            i = fluid.layers.fill_constant(shape=[1], dtype='int64', value=value)
...
def test_array_write():
    ...
    with fluid.unique_name.guard():
        with fluid.program_guard(train_program, startup_program):
            ...
            arr = fluid.layers.array_write(tmp, i=i)
            tmp = fluid.layers.fill_constant(shape=[3, 2], dtype='int64', value=7)
            i = fluid.layers.fill_constant(shape=[1], dtype='int64', value=10)
            # 将tmp写入数组arr下标为10的位置，并返回arr
            arr = fluid.layers.array_write(tmp, i=i)
            # 读取arr的下标10的位置的数据
...
def test_equal():
    ...
    with fluid.unique_name.guard():
        with fluid.program_guard(train_program, startup_program):
            ...
            out3 = fluid.layers.equal(x=label_cond, y=limit, cond=out_cond)  # out2=[False, True] out_cond=[False, True]
            place = fluid.CUDAPlace(0) if fluid.is_compiled_with_cuda() else fluid.CPUPlace()
            exe = fluid.Executor(place)
            exe.run(startup_program)
            compiled_prog = fluid.compiler.CompiledProgram(train_program)
...
def test_greater_equal():
    ...
    with fluid.unique_name.guard():
        with fluid.program_guard(train_program, startup_program):
            ...
            out1 = fluid.layers.greater_equal(x=label, y=limit)  # out1=[True, True]
            out2 = fluid.layers.greater_equal(x=label, y=limit1)  # out2=[True, True]
            out3 = fluid.layers.greater_equal(x=label_cond, y=limit, cond=out_cond)  # out3=[False, True]
            place = fluid.CUDAPlace(0) if fluid.is_compiled_with_cuda() else fluid.CPUPlace()
            exe = fluid.Executor(place)
            exe.run(startup_program)
            compiled_prog = fluid.compiler.CompiledProgram(train_program)
...
def test_greater_equal1():
    ...
    with fluid.unique_name.guard():
        with fluid.program_guard(train_program, startup_program):
            ...
            out1 = fluid.layers.greater_equal(x=label, y=limit)  # out1=[True, True]
            out2 = fluid.layers.greater_equal(x=label, y=limit1)  # out2=[True, True]
            out3 = fluid.layers.greater_equal(x=label_cond, y=limit, cond=None)  # out3=[False, True]
            place = fluid.CUDAPlace(0) if fluid.is_compiled_with_cuda() else fluid.CPUPlace()
            exe = fluid.Executor(place)
            exe.run(startup_program)
            compiled_prog = fluid.compiler.CompiledProgram(train_program)
...
def test_greater_than():
    ...
    with fluid.unique_name.guard():
        with fluid.program_guard(train_program, startup_program):
            ...
            limit1 = fluid.layers.assign(np.array([3, 1], dtype="int32"))
            label_cond = fluid.layers.assign(np.array([1, 3], dtype="int32"))
            out1 = fluid.layers.greater_than(x=label, y=limit)  # out1=[False, False]
            out2 = fluid.layers.greater_than(x=label, y=limit1)  # out2=[False, True]
            out3 = fluid.layers.greater_than(x=label_cond, y=limit, cond=None)  # out3=[False, True]
            place = fluid.CUDAPlace(0) if fluid.is_compiled_with_cuda() else fluid.CPUPlace()
            exe = fluid.Executor(place)
            exe.run(startup_program)
            compiled_prog = fluid.compiler.CompiledProgram(train_program)
...
def test_greater_than1():
    ...
    with fluid.unique_name.guard():
        with fluid.program_guard(train_program, startup_program):
            ...
            out1 = fluid.layers.greater_than(x=label, y=limit)  # out1=[False, False]
            out2 = fluid.layers.greater_than(x=label, y=limit1)  # out2=[False, True]
            out3 = fluid.layers.greater_than(x=label_cond, y=limit, cond=out_cond)  # out3=[False, True]
            place = fluid.CUDAPlace(0) if fluid.is_compiled_with_cuda() else fluid.CPUPlace()
            exe = fluid.Executor(place)
            exe.run(startup_program)
            compiled_prog = fluid.compiler.CompiledProgram(train_program)
...
def test_is_empty1():
    ...
    with fluid.unique_name.guard():
        with fluid.program_guard(train_program, startup_program):
            ...
            exe.run(startup_program)
            compiled_prog = fluid.compiler.CompiledProgram(train_program)
            res = exe.run(compiled_prog, feed={"a": np.array([])},
                          fetch_list=[x])[0][0]
            print(res)
...
def test_less_equal():
    ...
    with fluid.unique_name.guard():
        with fluid.program_guard(train_program, startup_program):
            ...
            limit1 = fluid.layers.assign(np.array([3, 1], dtype="int32"))
            label_cond = fluid.layers.assign(np.array([1, 3], dtype="int32"))
            out1 = fluid.layers.less_equal(x=label, y=limit)  # out1=[True, True]
            out2 = fluid.layers.less_equal(x=label, y=limit1)  # out2=[True, False]
            out3 = fluid.layers.less_equal(x=label_cond, y=limit, cond=out_cond)  # out3=[True, False]
            place = fluid.CUDAPlace(0) if fluid.is_compiled_with_cuda() else fluid.CPUPlace()
            exe = fluid.Executor(place)
            exe.run(startup_program)
            compiled_prog = fluid.compiler.CompiledProgram(train_program)
            res = exe.run(compiled_prog,
                          fetch_list=[out1, out2, out_cond])
            expect1 = np.array([True, True])
            expect2 = np.array([True, False])
...
def test_less_than():
    ...
    with fluid.unique_name.guard():
        with fluid.program_guard(train_program, startup_program):
            ...
            limit1 = fluid.layers.assign(np.array([3, 1], dtype="int32"))
            label_cond = fluid.layers.assign(np.array([1, 3], dtype="int32"))
            out1 = fluid.layers.less_than(x=label, y=limit)  # out1=[False, False]
            out2 = fluid.layers.less_than(x=label, y=limit1)  # out2=[False, False]
            out3 = fluid.layers.less_than(x=label_cond, y=limit, cond=out_cond)  # out3=[True, False]
            place = fluid.CUDAPlace(0) if fluid.is_compiled_with_cuda() else fluid.CPUPlace()
            exe = fluid.Executor(place)
            exe.run(startup_program)
            compiled_prog = fluid.compiler.CompiledProgram(train_program)
...
def test_less_than1():
    ...
    with fluid.unique_name.guard():
        with fluid.program_guard(train_program, startup_program):
            ...
            out1 = fluid.layers.less_than(x=label, y=limit)  # out1=[False, False]
            out2 = fluid.layers.less_than(x=label, y=limit1)  # out2=[False, False]
            out3 = fluid.layers.less_than(x=label_cond, y=limit)  # out3=[True, False]
            place = fluid.CUDAPlace(0) if fluid.is_compiled_with_cuda() else fluid.CPUPlace()
            exe = fluid.Executor(place)
            exe.run(startup_program)
            compiled_prog = fluid.compiler.CompiledProgram(train_program)
...
def test_not_equal():
    ...
    with fluid.unique_name.guard():
        with fluid.program_guard(train_program, startup_program):
            ...
            limit1 = fluid.layers.assign(np.array([3, 1], dtype="int32"))
            label_cond = fluid.layers.assign(np.array([1, 3], dtype="int32"))
            out1 = fluid.layers.not_equal(x=label, y=limit)  # out1=[False, False]
            out2 = fluid.layers.not_equal(x=label, y=limit1)  # out2=[False, True]
            out3 = fluid.layers.not_equal(x=label_cond, y=limit, cond=out_cond)  # out3=[True, True]
            place = fluid.CUDAPlace(0) if fluid.is_compiled_with_cuda() else fluid.CPUPlace()
            exe = fluid.Executor(place)
            exe.run(startup_program)
            compiled_prog = fluid.compiler.CompiledProgram(train_program)
...
def test_not_equal1():
    ...
    with fluid.unique_name.guard():
        with fluid.program_guard(train_program, startup_program):
            ...
            out1 = fluid.layers.not_equal(x=label, y=limit)  # out1=[False, False]
            out2 = fluid.layers.not_equal(x=label, y=limit1)  # out2=[False, True]
            out3 = fluid.layers.not_equal(x=label_cond, y=limit)  # out3=[True, True]
            place = fluid.CUDAPlace(0) if fluid.is_compiled_with_cuda() else fluid.CPUPlace()
            exe = fluid.Executor(place)
            exe.run(startup_program)
            compiled_prog = fluid.compiler.CompiledProgram(train_program)
