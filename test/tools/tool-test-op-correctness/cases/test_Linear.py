...
def test_dygraph():
    ...
    for place in places:
        for t in types:
            ...
            adam = paddle.optimizer.Adam(parameters=[linear.weight, linear.bias], 
            ...
            for i in range(10):
               ...
               out.backward()
               adam.step()
            res_weight = np.array([[1.9000001, 1.9000001, 1.9000001, 1.9000001],
                                   [1.9000001, 1.9000001, 1.9000001, 1.9000001]])
            res_bias = np.array([-0.09999976, -0.09999976, -0.09999976, -0.09999976])
            res_out = np.array([[13.280001, 13.280001, 13.280001, 13.280001],
    ...
    for place in places:
       for t in types:
          ...
          with fluid.program_guard(main_program=main_program, startup_program=startup_program):
            ...
            linear = paddle.nn.Linear(2, 4, 
                                      weight_attr=fluid.initializer.ConstantInitializer(value=2.0), bias_attr=None)
            adam = paddle.optimizer.Adam(parameters=[linear.weight, linear.bias], 
                                         learning_rate=0.01, beta1=0.9, beta2=0.999, epsilon=1e-08)
            ...
            for i in range(10):
               out, weight, bias = exe.run(main_program, feed={'x':x}, fetch_list=[output, linear.weight, linear.bias])
            res_weight = np.array([[1.9000002, 1.9000002, 1.9000002, 1.9000002],
