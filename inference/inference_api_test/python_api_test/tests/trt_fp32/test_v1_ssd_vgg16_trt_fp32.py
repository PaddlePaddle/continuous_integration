...
def inference_ssd_vgg16(img, model_path, params_path, with_ir=True, use_trt=True):
    """
    inference_ttfnet
    Args:
        img: numpy img
...
def test_ssd_vgg16():
    ...
    with_ir_data = inference_ssd_vgg16(img, model_path, params_path)
    no_ir_data = inference_ssd_vgg16(img, model_path, params_path, with_ir=False, use_trt=False)
    # npy_result = test_model.npy_result_path("cv_detect_model")

    test_model.test_diff(no_ir_data[0], with_ir_data[0], diff_standard)

    # det image with box
    # image_preprocess.draw_bbox(img_name, without_lr_data[0], save_name="ssd_vgg16.jpg")
