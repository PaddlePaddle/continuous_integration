...
class Predictor(object):
    ...
    def __init__(self,
        ...
        try:
            if predictor_mode == "Analysis":
                ...
                predictor0 = fluid.core.create_paddle_predictor(analysis_predictor_config)
                ...
                self.analysis_predictor = fluid.core.create_paddle_predictor(analysis_predictor_config)
    ...
    def analysis_predict(self, json_dir, repeats=1):
        ...
        for i in range(repeats):
            ...
            for i, output_data in enumerate(output_names):
                logger.info("====> output_names[{0}] = {1} <====".format(i, output_data))
    ...
    def native_predict(self, json_dir, repeats=1):
        ...
        cost_time = np.array(cost_time)
        ave_time = cost_time.mean()
        output_data = [x.as_ndarray() for x in outputs] # return list[numpy.ndarray]
