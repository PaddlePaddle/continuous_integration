...
import image_classfication.utils.reader_cv2 as reader
sys.path.append("./image_classfication")
...
DATA_DIR = './data/dist_data/ImageNet'
...
class TestDistVgg16(FleetDistRunnerBase):
    ...
    def net(self, args=None):
        ...
        from image_classfication.models.vgg import VGG16
        from image_classfication.train import parser
        from image_classfication.train import optimizer_setting
        parser.add_argument(
        ...
        parser.add_argument('--run_params', type=str, required=False, default='{}')
        ...
        dist_strategy.forward_recompute = args.run_params["recompute"] if "recompute" in args.run_params else False
        ...
        shuffle_seed = 1
        train_reader = reader.train(settings=args, data_dir=DATA_DIR, pass_id_as_seed=shuffle_seed)
        self.py_reader.decorate_paddle_reader(paddle.batch(train_reader, batch_size=self.batch_size))
