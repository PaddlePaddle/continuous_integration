...
if __name__ == "__main__":
    ...
    try:
        ...
        os.system('echo "GPU yolov3_darknet inference error" >> log.txt')
    else:
        max_diff = np.max(np.abs(data - ir_optim_data))
        if max_diff < 1e-6:
            print('GPU yolov3_darknet ok')
        else:
            print('GPU yolov3_darknet failed, diff is ', max_diff)
            os.system('echo "GPU yolov3_darknet failed,diff is "' + str(max_diff) + ' >> log.txt')
