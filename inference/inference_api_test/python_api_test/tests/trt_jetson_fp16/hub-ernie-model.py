...
if __name__ == "__main__":
    try:
        ...
        os.system('echo "TensorRT FP16 hub-ernie inference error" >> log.txt')
    else:
        max_diff = np.max(np.abs(data - ir_optim_data))
        if max_diff < 1e-6:
            print('TensorRT FP16 hub-ernie ok')
        else:
            print('TensorRT FP16 hub-ernie failed, diff is ', max_diff)
            os.system('echo "TensorRT FP16 hub-ernie failed,diff is "' + str(max_diff) + '>> log.txt')
