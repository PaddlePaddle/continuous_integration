...
def main(ir_optim=True):
    ...
    np.random.seed(2)
    fake_input2 = np.random.randn(args.batch_size, 3, 2).astype("float32")
...
def parse_args():
    ...
    parser.add_argument("--model_file",default='../../Data/det_inference_model/faster_rcnn_r50_fpn_1x_coco/model.pdmodel' ,type=str, help="model filename")
    parser.add_argument("--params_file", default='../../Data/det_inference_model/faster_rcnn_r50_fpn_1x_coco/model.pdiparams',type=str, help="parameter filename")
...
if __name__ == "__main__":
    try:
        ...
        os.system('echo "TensorRT FP16 faster_rcnn_r50_fpn_1x_coco inference error" >> log.txt')
    else:
        max_diff = np.max(np.abs(data - ir_optim_data))
        if max_diff < 1e-6:
            print('TensorRT FP16 faster_rcnn_r50_fpn_1x_coco ok')
        else:
            print('TensorRT FP16 faster_rcnn_r50_fpn_1x_coco failed, diff is ', max_diff)
            os.system('echo "TensorRT FP16 faster_rcnn_r50_fpn_1x_coco failed,diff is "' + str(max_diff) + '>> log.txt')
