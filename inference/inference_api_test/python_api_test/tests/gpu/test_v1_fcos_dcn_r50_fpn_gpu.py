...
def inference_fcos_dcn_r50_fpn(img, model_path, params_path):
    """
    inference_fcos_dcn_r50_fpn
    Args:
        img: numpy img
...
@pytest.mark.p0
def test_fcos_dcn_r50_fpn():
    """
    test_fcos_dcn_r50_fpn
    Args:
        None
    Returns:
        None
    ...
    with_lr_data = inference_fcos_dcn_r50_fpn(img, model_path, params_path)
    npy_result = test_model.npy_result_path("cv_detect_model")
    test_model.test_diff(npy_result, with_lr_data[0], diff_standard)
    # det image with box
    # np.save("fcos_dcn_r50_fpn.npy",with_lr_data[0])
    # image_preprocess.draw_bbox(image_path, with_lr_data[0], save_name="fcos_dcn_r50_fpn.jpg")
