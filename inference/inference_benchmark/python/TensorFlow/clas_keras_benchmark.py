...
def prepare_model(args):
    ...
    if args.use_trt and args.trt_precision == "fp32":
        ...
        converter = trt.TrtGraphConverterV2(
            input_saved_model_dir='{}_saved_model'.format(args.model_name),
            conversion_params=conversion_params)
        converter.convert()
        converter.save(
            output_saved_model_dir='{}_saved_model_TFTRT_FP32'.format(
                args.model_name))
        ...
        converter.convert()
        converter.save(
            output_saved_model_dir='{}_saved_model_TFTRT_FP16'.format(
                args.model_name))
        ...
        converter.save(
            output_saved_model_dir='{}_saved_model_TFTRT_INT8'.format(
                args.model_name))
...
def run_demo():
    ...
    if args.use_trt:
        if args.trt_precision == "fp32":
            total_time = benchmark_tftrt(
                args, '{}_saved_model_TFTRT_FP32'.format(args.model_name))
        elif args.trt_precision == "fp16":
            total_time = benchmark_tftrt(
                args, '{}_saved_model_TFTRT_FP16'.format(args.model_name))
        elif args.trt_precision == "int8":
            total_time = benchmark_tftrt(
                args, '{}_saved_model_TFTRT_INT8'.format(args.model_name))
    else:
        total_time = benchmark_tftrt(args,
                                     '{}_saved_model'.format(args.model_name))
