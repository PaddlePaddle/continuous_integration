...
def test_conv1d2():
    ...
    if platform.system() == "Linux":
        ...
        stride = 1
        padding = 0
        res = np.array([[[2.9210, 2.5172, 3.0026], [2.9210, 2.5172, 3.0026],
                         [2.9210, 2.5172, 3.0026]],
                        [[2.7743, 2.2806, 2.7643], [2.7743, 2.2806, 2.7643],
...
def test_conv1d5():
    ...
    padding = 0
    groups = 3
    res = np.array([[[1.1094], [1.1094], [1.0751], [1.0751], [2.1321],
                     [2.1321]], [[0.5946], [0.5946], [1.7757], [1.7757],
                                 [1.5914], [1.5914]]])
    obj.run(res=res,
            data=x,
            in_channels=in_channels,
...
def test_conv1d6():
    ...
    stride = 1
    padding = 0
    res = np.array([[[2.7216, 2.5869], [2.7216, 2.5869]], [[2.4732, 2.4003],
                                                           [2.4732, 2.4003]]])
    obj.run(res=res,
            data=x,
            in_channels=in_channels,
...
def test_conv1d7():
    ...
    padding = 0
    data_format = "NLC"
    res = np.array([[[2.7216, 2.5869], [2.7216, 2.5869]],
                    [[2.4732, 2.4003], [2.4732, 2.4003]]]).transpose(0, 2, 1)
    obj.run(res=res,
            data=x,
            in_channels=in_channels,
            out_channels=out_channels,
