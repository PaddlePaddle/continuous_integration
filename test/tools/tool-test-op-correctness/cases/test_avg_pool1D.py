...
* @file test_avg_pool1D.py
* @author zhengtianyu
* @date 2020-08-28 16:59:21
* @brief test_avg_pool1D
*
**************************************************************************/
"""
...
obj = TestAvgPool1D(paddle.nn.AvgPool1D)
...
def test_avg_pool1D_base():
    ...
    obj.base(res=res, data=x, kernel_size=kernel_size, stride=stride, padding=padding)
...
def test_avg_pool1D2():
    ...
    padding = 1
    exclusive = False
    res = np.array([[[-2.5149, -3.0151, -1.3908, -3.2876, 1.3070, 0.5544, -7.9530,
          -0.2709, 4.5325],
         [1.8045, 1.6704, 4.5161, 3.5843, -5.2704, -5.6904, -4.8496,
           1.4680, 4.8317],
         [3.8063, 3.7469, -1.0498, -1.4775, 1.7217, -0.3135, -1.2945,
          -2.3477, -3.5755]],
        [[-2.9882, -7.1761, 0.3469, 0.0921, -3.4473, 3.2253, 6.9329,
           7.9186, 3.2157],
         [0.2755, -1.4097, -3.1454, -5.6699, -3.6506, -2.7829, -5.3897,
           1.3585, 3.4061],
         [-1.3744, -2.9662, 0.5122, -2.5789, -4.9590, 2.0396, 5.0479,
           0.1446, -2.5876]]])
    obj.run(res=res, data=x, kernel_size=kernel_size, stride=stride, padding=padding, exclusive=exclusive)
