...
if __name__ == "__main__":
    try:
        ...
        os.system('echo "GPU mask_rcnn_r50_1x inference error" >> log.txt')
    else:
        max_diff = np.max(np.abs(data - ir_optim_data))
        if max_diff < 1e-6:
            print('GPU mask_rcnn_r50_1x ok')
        else:
            print('GPU mask_rcnn_r50_1x failed, diff is ', max_diff)
            os.system('echo "GPU mask_rcnn_r50_1x failed,diff is "' + str(max_diff) + '>> log.txt')
