...
for config_file in `find . -name "*train_infer_python.txt"`; do
    for mode in $test_mode; do
        ...
        mv test_tipc/output "test_tipc/output_"$(echo $config_file | tr "/" "_")"_"$mode || echo "move output error"
        mv test_tipc/output "test_tipc/data"$(echo $config_file | tr "/" "_")"_"$mode || echo "move data error"
