...
if __name__ == "__main__":
    try:
        ...
        os.system('echo "GPU ch_rec_use_space_char inference error" >> log.txt')
    else:
        max_diff = np.max(np.abs(data - ir_optim_data))
        if max_diff < 1e-6:
            print('GPU ch_rec_use_space_char ok')
        else:
            print('GPU ch_rec_use_space_char failed, diff is ', max_diff)
            os.system('echo "GPU ch_rec_use_space_char failed,diff is "' + str(max_diff) + '>> log.txt')
