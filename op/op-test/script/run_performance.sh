...
nvidia-docker exec -e BUILD_VCS_NUMBER=${BUILD_VCS_NUMBER} -e TEST_ROOT_DIR=${TEST_ROOT_DIR} paddle_op_ce /bin/bash ./command_performance.sh
...
cat ${TEST_ROOT_DIR}/ce/${BUILD_VCS_NUMBER}/performance/${BUILD_VCS_NUMBER} | while read line
do
    array=(${line//	/ })
    case_name=${array[0]}
    performance=${array[1]}
