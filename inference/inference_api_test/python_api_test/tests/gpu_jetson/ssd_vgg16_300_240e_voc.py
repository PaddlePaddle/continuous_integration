...
if __name__ == "__main__":
    ...
    try:
        ...
        os.system('echo "GPU ssd_vgg16_300_240e_voc inference error" >> log.txt')
    else:
        max_diff = np.max(np.abs(data - ir_optim_data))
        if max_diff < 1e-6:
            print('GPU ssd_vgg16_300_240e_voc ok')
        else:
            print('GPU ssd_vgg16_300_240e_voc failed, diff is ', max_diff)
            os.system('echo "GPU ssd_vgg16_300_240e_voc failed,diff is "' + str(max_diff) + '>> log.txt')
