...
def parse_args():
    ...
    parser.add_argument("--model_file",default='../../Data/python-model-infer/Detection/yolov3_darknet/model/__model__' ,type=str, help="model filename")
    parser.add_argument("--params_file", default='../../Data/python-model-infer/Detection/yolov3_darknet/model/__params__',type=str, help="parameter filename")
...
if __name__ == "__main__":
    ...
    try:
        ...
        os.system('echo "TensorRT FP16 yolov3_darknet inference error" >> log.txt')
    else:
        max_diff = np.max(np.abs(data - ir_optim_data))
        if max_diff < 1e-6:
            print('TensorRT FP16 yolov3_darknet ok')
        else:
            print('TensorRT FP16 yolov3_darknet failed, diff is ', max_diff)
            os.system('echo "TensorRT FP16 yolov3_darknet failed,diff is "' + str(max_diff) + ' >> log.txt')
