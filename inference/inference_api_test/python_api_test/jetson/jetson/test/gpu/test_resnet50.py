...
def inference_resnet50(img, model_path, params_path):
    """
    inference_ttfnet
    Args:
        img: numpy img
...
def test_resnet50():
    ...
    with_lr_data = inference_resnet50(img, model_path, params_path)
    npy_result = test_model.npy_result_path("cv_class_model")
    test_model.test_diff(npy_result, with_lr_data[0], diff_standard)
    #np.save("resnet50.npy",with_lr_data[0])
    #show class
    #print(np.argmax(without_lr_data[0][0]))
